<%-- 
    Document   : ventas
    Created on : 23 oct 2025, 9:59:17 p.m.
    Author     : Admin
--%>
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse">
      <ul class="navbar-nav me-auto">
        ...
        <li class="nav-item"><a class="nav-link active" href="ventas.jsp">Ventas</a></li>
      </ul>
    </div>
  </div>
</nav>
...
<div class="container py-4">
  <h3 class="mb-3">Venta (Maestro–Detalle)</h3>

  <form action="sr_venta" method="post" id="frmVenta" class="row g-3">

    <!-- Maestro -->
    <div class="col-md-2">
      <label class="form-label">No. Factura</label>
      <input type="number" class="form-control" name="txt_nofactura" required>
    </div>
    <div class="col-md-2">
      <label class="form-label">Serie</label>
      <input type="text" class="form-control" name="txt_serie" maxlength="1" value="A">
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-5">
      ...
      <select class="form-select" name="drop_cliente" required>
        <%
          ...
          Map<String,String> cli = c.leerDrop();
          for (String id: cli.keySet()){
            out.println("<option value='"+id+"'>"+cli.get(id)+"</option>");
          }
        %>
      </select>
    </div>
    <div class="col-md-5">
      ...
      <select class="form-select" name="drop_empleado" required>
        <%
          ...
          Map<String,String> emp = e.leerDrop();
          for (String id: emp.keySet()){
            out.println("<option value='"+id+"'>"+emp.get(id)+"</option>");
          }
        %>
      </select>
    </div>
    ...
    <div class="col-12">
      <button class="btn btn-primary" name="btn_crear" value="crear">Guardar Venta</button>
    </div>
  </form>
  ...
  <div class="table-responsive">
    <table class="table table-hover">
      <thead class="table-dark">
        <tr><th>ID</th><th>NoFactura</th><th>Serie</th><th>Fecha</th><th>Cliente</th><th>Empleado</th><th>Total</th></tr>
      </thead>
      <tbody>
        <%
          Venta v = new Venta();
...
<script>
  // Catálogo de productos (id, nombre, marca, precio, existencia)
  const catalogo = [
    <% 
      Producto p = new Producto();
      java.util.List<String[]> prods = p.leerDropProductos();
      for (int i=0; i<prods.size(); i++){
        String[] r = prods.get(i);
        // r[0]=id, r[1]=producto, r[2]=marca, r[3]=precio, r[4]=existencia
        out.print("{id:"+r[0]+", nom:\""+r[1].replace("\"","\\\"")+"\", marca:\""+r[2].replace("\"","\\\"")+"\", precio:"+r[3]+", exist:"+r[4]+"}");
  ...
  function optionHtml(o){
    return '<option value="'+o.id+'" data-precio="'+o.precio+'" data-exist="'+o.exist+'">'
           + o.nom + ' (' + o.marca + ')</option>';
  }
  ...
  function filaDetalle(){
    ...
    var html = ''
      ...
      +   '<td class="text-end">'
      +     '<input type="number" step="0.01" min="0" class="form-control text-end w-100p precio" '
      +            'name="detail_precio" value="">'
      +   '</td>'
      +   '<td class="text-end">'
      +     '<input type="number" min="1" class="form-control text-end w-100p cant" '
